...
class SimulationEvaluator(BaseEvaluator):
    ...
    def _calculate_review_metrics(
        ...
        for sim_emotion, real_emotion in zip(simulated_emotions, real_emotions):
            emotion_error_single = self._calculate_emotion_error(sim_emotion, real_emotion) # type: ignore
...
async def evaluation(to_evaluate: Any, datasets_path: str, metadata: Dict):
    ...
    if metadata['mode'] == 'inference':
        # set ground truth to inference tasks
        with open(datasets_path + "/inference_tasks_groundtruth.json", "r") as f:
            test_tasks = json.load(f)
        ground_truths = {}
        for task in test_tasks:
            ground_truths[task['task_id']] = task['ground_truth']
        for result in to_evaluate:
            task_id = result['id']
            result['ground_truth'] = ground_truths[task_id]
    for result in to_evaluate:
        if result['context']['target'] == 'recommendation':
            recommendation_ground_truth.append(result['ground_truth']['item_id'])
            recommendation_predictions.append(result['result']['item_list'])
        elif result['context']['target'] == 'review_writing':
            simulation_ground_truth.append(result['ground_truth'])
            simulation_predictions.append(result['result'])

    # ========================    
    # calculate recommendation metrics
    # ========================
    recommendation_evaluator = RecommendationEvaluator()
    recommendation_metrics = recommendation_evaluator.calculate_hr_at_n(recommendation_ground_truth, recommendation_predictions)

    # ========================    
    # calculate simulation metrics
    # ========================
    simulation_evaluator = SimulationEvaluator()
    simulation_metrics = simulation_evaluator.calculate_metrics(simulation_ground_truth, simulation_predictions)

    # ========================    
    # calculate final score
    # ========================
    final_score = ((recommendation_metrics.average_hit_rate + simulation_metrics.overall_quality) / 2) * 100
    return {
        "recommendation_metrics": recommendation_metrics.__dict__,
        "simulation_metrics": simulation_metrics.__dict__,
