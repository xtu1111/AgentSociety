...
export const RightPanel = observer(() => {
  ...
  const handleSelectSubmit = async () => {
    const res = await fetchCustom(
      `/api/experiments/${store.expID}/agents/${agent?.id}/survey`,
      {
        method: 'POST',
              ...
              const results: any[] = [
                {
                  ...
                  content: (
                    <div>
                      {t('replay.chatbox.survey.surveyName')}: {survey.name}{' '}
                      <Button
                        type='link'
                        onClick={() => {
  ...
  // ✅ 修复：只声明一次 entries，并先做数据净化，避免 TS2451
  const renderMetrics = () => {
    const metrics = store.metrics;

    // 过滤掉 step/value 非有限数值的点
    ...
    const entries: [string, ApiMetric[]][] = Array.from(metrics.entries())
      .map(([key, values]) => [key, sanitizeMetric(values)] as [string, ApiMetric[]])
    ...
    return (
      <div style={{ overflow: 'auto', height: '70vh', width: '100%' }}>
        {entries.map(([key, series]) => {
          const x = series.map(v => v.step);
          const y = series.map(v => v.value);
          return (
            <div key={key}>
              <Plot
                data={[
                ...
                layout={{
                  title: { text: key, font: { size: 16, family: 'Arial' } },
                  autosize: true,
                  height: 200,
                  margin: { l: 30, r: 10, t: 30, b: 30 },
                  xaxis: {
                    title: { text: t('replay.chatbox.metrics.step'), font: { size: 12 } },
                    showgrid: true,
                    gridcolor: '#f0f0f0'
                  },
                  yaxis: {
                    title: { text: t('replay.chatbox.metrics.value'), font: { size: 12 } },
                    showgrid: true,
                    gridcolor: '#f0f0f0'
                  },
